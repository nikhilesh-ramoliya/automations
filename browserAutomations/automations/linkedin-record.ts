...
const DEFAULT_LANATUS_COMPANY_URL =
  "https://www.linkedin.com/company/lanatus/";
...
/** Start URLs from env: RECORD_START_URLS, or LANATUS_COMPANY_URL / LINKEDIN_COMPANY_URL, else Lanatus default. */
function resolveStartUrls(): string[] {
  const fromList = process.env.RECORD_START_URLS?.trim();
  if (fromList) {
    return fromList
      .split(",")
      .map((s) => s.trim())
...
async function waitForEnterOrClose(
  ...
  if (!process.stdin.isTTY) {
    console.log(
      `${message}\n(No TTY — close the browser window to end the recording.)`,
    );
    await closedPromise;
    return "closed";
  }
  ...
  const enterPromise = new Promise<"enter">((resolve) => {
    rl.question(`${message}\nPress Enter here to end the recording… `, () => {
      resolve("enter");
    });
  });
...
async function installDomLogging(
  ...
  await context.exposeFunction("__sessionLog", (payload: DomEventPayload) => {
    log({
      ts: new Date().toISOString(),
      ...payload,
    });
...
async function openStartUrls(
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    if (i > 0) attachPageListeners(page, log);
    ...
    try {
      ...
      console.warn(
        `Failed to open start URL ${url}:`,
        err instanceof Error ? err.message : err,
      );
      log({
        ts: new Date().toISOString(),
        type: "start_url_error",
        url,
...
async function main(): Promise<void> {
  ...
  const startUrls = resolveStartUrls();
  const logPath = path.join(
    LOGS_DIR,
    `recording-${timestampForFilename()}.jsonl`,
  );
  ...
  console.log("\n════════════════════════════════════════");
  console.log(" LinkedIn action recorder");
  console.log(` Log file: ${relativeLog}`);
  ...
  acquireLinkedInJobLock("record:linkedin");
  ...
  try {
    ...
    console.warn(
      "Initial feed navigation timed out or failed; checking auth…",
      err instanceof Error ? err.message : err,
    );
  }
  ...
  await openStartUrls(context, page, startUrls, writer.log);

  console.log(
    "\nBrowser open. Perform your LinkedIn actions manually — clicks/inputs are logged.",
  );
  console.log(`Recording → ${relativeLog}\n`);

  const reason = await waitForEnterOrClose(
    "Interact in the browser (e.g. Invite to follow Lanatus).",
    closedPromise,
  );
  ...
  console.log(` ${relativeLog}`);
  console.log(" Generate a workflow with:");
  console.log(
    `   npm run workflow:from-recording -- ${relativeLog.replace(/\\/g, "/")}`,
  );
  console.log("════════════════════════════════════════\n");
